...
class UserController extends FrontendController
{
    ...
    public function changePasswordUpdate(Request $request)
    {
        if (!(Hash::check($request->get('current-password'), Auth::user()->password))) {
            ...
            return redirect()->back()->with("error", __("Votre mot de passe actuel ne correspond pas au mot de passe que vous avez fourni. Veuillez réessayer."));
        }
        if (strcmp($request->get('current-password'), $request->get('new-password')) == 0) {
            //Current password and new password are same
            return redirect()->back()->with("error", __("Le nouveau mot de passe ne peut pas être identique à votre mot de passe actuel. Veuillez choisir un mot de passe différent."));
        }
        $request->validate([
        ...
        $user->save();
        return redirect()->back()->with('success', __('Mot de passe changé avec succès !'));
    }
    ...
    public function upgradeVendor(Request $request){
        ...
        if(!empty($vendorRequest)){
            return redirect()->back()->with('warning', __('Vous venez de soumettre la demande de devenir vendeur. Veuillez attendre l\'approbation de l\'administrateur.'));
        }
    ...
    public function permanentlyDelete(Request $request){
        ...
        return back()->with('error',__('Erreur. Vous ne pouvez pas supprimer définitivement.'));
