...
class PermissionController extends AdminController
{
    ...
    public function create(Request $request)
    {
        if (!empty($request->input())) {
            ...
            if ($row->save()) {
                ...
                return redirect(route('user.admin.permission.index'))->with('success', __('Permission created'));
            }
        } else {
            $row = new Permission();
            $row->fill([
    ...
    public function edit(Request $request, $id)
    {
        ...
        if (empty($row)) {
            return redirect(route('user.admin.permission.index'));
        }
        if (!empty($request->input())) {
            $row->fill($request->input());
            if ($row->save()) {

                return redirect(route('user.admin.permission.index'))->with('success', __('Permission updated'));
            }
        }
        $data = [
            'row' => $row
