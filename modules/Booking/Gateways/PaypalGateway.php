...
//                    $booking->pay_now = (float)($oldPaynow - $data['originalAmount'] < 0 ? 0 : $oldPaynow - $data['originalAmount']);
                ...
                return redirect($booking->getDetailUrl())->with("success", __("You payment has been processed successfully! Bravo!"));
            } else {
    ...
    public function confirmNormalPayment()
    {
        ...
        if($payment){
            if($payment->status == 'cancel'){
                return [false,__("Your payment has been canceled. Why?")];
            }
        }
        return [false];
    }
